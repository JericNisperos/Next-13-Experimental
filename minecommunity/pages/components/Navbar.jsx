...
import { faBars, faGamepad, faHamburger, faHouse, faPeopleGroup, faPerson, faShop, faSort, faSortAlphaAsc, faSortAmountAsc, faSortAmountDesc, faSortAmountUp } from "@fortawesome/free-solid-svg-icons";
  ...
  function NavItems({ href, icon, title }) {
    return (
      <Link href={href} onClick={() => setNavMobile(false)}>
        <li className={`flex items-center gap-x-2 text-white font-extrabold  px-3 py-2 ${router.pathname === href ? "bg-blue-500  rounded-md border-solid border-2 border-blue-500 hover:text-white" : "hover:text-blue-400"}`}>
            ...
            <div className="justify-between px-4 lg:max-w-5 lg:items-center lg:flex lg:px-2 lg:mx-16 ">
              <div alt="Icon and Name" className="flex items-center justify-between py-3 md:py-5 md:block flex-no-wrap">
                <motion.div whileHover={{ scale: 1.1, transition: { duration: 0.5 } }} transition={{ type: "spring", stiffness: 200, damping: 10 }} className="flex  items-center ">
                  ...
                  <Link href="/" className="md:text-xl block sm:block md:hidden lg:block text-sm text-white font-bold ml-2 ">
                    MineCommunity
                  </Link>
                </motion.div>
