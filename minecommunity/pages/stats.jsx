...
function stats() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <>
        {/* <div className="mx-48 flex flex-col md:flex-row items-center justify-end text-center md:text-left mb-16 mt-12 border">
        <input />
      </div> */}
        <div class="mx-auto md:mx-48 flex flex-col md:flex-row items-center md:text-left gap-10 mb-16 mt-12">
          <motion.div className="w-full cursor-default justify-between grid grid-cols-1 md:grid-cols-3" initial={{ opacity: 0, y: "10%" }} animate={{ opacity: 1, y: 0, transition: { duration: 0.5, delay: 0.2 } }}>
            <div class="col-span-2 text-lg md:text-5xl font-bold text-white">
              {/* Server Statistics */}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3  mx-4 md:mx-32 gap-x-4 gap-y-4 ">
              <div className="col-span-1 md:col-span-2 rounded-lg bg-blue-100">
                ...
                <div className="justify-center mt-8 font-bold ">
                  <DataItems icon={<IoMdClock />} title="Server Uptime:" value={stats.numbers?.current_uptime} />
                  <DataItems icon={<IoPeople />} title="Total Players:" value={stats.numbers?.total_players.toLocaleString("en-US")} />
                  <DataItems icon={<IoPeople />} title="Regular Players:" value={stats.numbers?.regular_players} />
                  <DataItems icon={<IoPeople />} title="Currently Online:" value={stats.numbers?.online_players} />
                  <div className="border border-gray-700 mx-8"></div>
                  <DataItems icon={<AiOutlineLineChart />} title={`Today's Peak (${dateToday}):`} value={`${stats.numbers?.last_peak_players} (Players)`} />
                  <DataItems icon={<AiOutlineLineChart />} title={`Best Peak (${stats.numbers?.best_peak_date.split(",")[0]}):`} value={`${stats.numbers?.best_peak_players} (Players)`} />
                  <div className="border border-gray-700 mx-8"></div>
                  <DataItems icon={<GiBroadsword />} title="Overall Player Kills:" value={`${stats.numbers?.player_kills.toLocaleString("en-US")} Kills`} />
                  <DataItems icon={<GiDeathSkull />} title="Overall Player Deaths:" value={`${stats.numbers?.deaths.toLocaleString("en-US")} Deaths`} />

                  <DataItems icon={<GiFishMonster />} title="Overall Mob Kills:" value={`${stats.numbers?.mob_kills.toLocaleString("en-US")} Kills`} />
                </div>
              </div>
              ...
              <div className="col-span-1 rounded-lg bg-blue-100 shadow-md shadow-white">
                ...
                <div className="justify-center mt-8 font-bold ">
                  <DataItems icon={<IoIosPeople />} title="Unique Players:" value={stats.last_7_days?.unique_players} />
                  <DataItems icon={<IoPeople />} title="Unique Players / Day:" value={stats.last_7_days?.unique_players_day} />

                  <div className="flex text-lg md:text-2xl my-2 py-2 text-white bg-blue-400">
                    <div className="flex mx-auto ">New Players</div>
                  </div>

                  <DataItems icon={<MdPeopleAlt />} title="New Players:" value={stats.last_7_days?.new_players} />
                  <DataItems icon={<MdPeopleAlt />} title="New Players Retention:" value={stats.last_7_days?.new_players_retention} />
                  <DataItems icon={<MdPeopleAlt />} title="Retention Rate:" value={stats.last_7_days?.new_players_retention_perc} />

                  <div className="flex text-lg md:text-2xl my-2 py-2 text-white bg-blue-400">
                    <div className="flex mx-auto ">Ticks per Second (TPS)</div>
                  </div>

                  <DataItems icon={<BsArrowClockwise />} title="Average TPS:" value={stats.last_7_days?.average_tps} />
                  <DataItems icon={<BsFillClockFill />} title="Low Spike TPS:" value={stats.last_7_days?.low_tps_spikes} />
                  <DataItems icon={<BiPowerOff />} title="Downtime:" value={stats.last_7_days?.downtime} />
                </div>
              </div>
            </div>
          </motion.div>
