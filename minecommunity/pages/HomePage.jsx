    ...
    <div className="text-white ">
      ...
      <motion.div initial={{ y: "10%" }} animate={{ y: 0 }} transition={{ duration: 0.5 }} className="grid grid-cols-1 lg:grid-cols-3 md:mx-48 mt-8 md:mt-12">
        <div className="rounded-lg flex flex-col justify-center items-center mx-4 ">
          <h1 className="text-red-400  text-2xl md:text-6xl font-extrabold justify-center text-center">⚔︎ 1.19.4 Survival Guild Realms</h1>
        ...
        <div className="rounded-lg col-span-2 mt-8 md:mt-0">
          <div className="object-scale-down md:h-full md:w-full md:relative rounded-md">
            {images.map((image, index) => (
              <motion.img
                ...
                src={image.src}
                alt={image.alt}
                className={`object-scale-down bg-yellow-300 absolute w-full rounded-lg h-50 md:object-cover transition-opacity duration-500 ${activeImage === index ? "opacity-100" : "opacity-0"}`}
                variants={variants}
